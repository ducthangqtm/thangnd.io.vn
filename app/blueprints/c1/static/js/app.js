...
async function refreshData() {
    try {
        const [scheduleRes, standingsRes, arsenalScheduleRes] = await Promise.all([
            fetch("/c1/api/schedule"),
            fetch("/c1/api/standings"),
            fetch("/c1/api/arsenal/schedule")
        ]);
            ...
            <tr>
                ...
                <td class="col-team">
                    <div class="team-cell">
                        ...
                        <span class="team-cell-name">${s.team} ${s.group ? `<small style="color:var(--text-muted); font-weight:500; font-size:0.65rem; margin-left:4px;">(${s.group})</small>` : ''}</span>
                    </div>
                </td>
...
function renderArsenalSchedule() {
    ...
    if (!state.arsenalMatches || state.arsenalMatches.length === 0) {
        container.innerHTML = `<div class="loading-state">Không tìm thấy lịch thi đấu nào cho Arsenal ở C1.</div>`;
        return;
    }
