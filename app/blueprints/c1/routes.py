...
def api_get_standings():
    ...
    if children:
        # Note: Champions league might have multiple groups or a single league phase table
        # We will parse all groups/entries.
        for child in children:
            standings_data = child.get("standings", {}).get("entries", [])
            group_name = child.get("name", "") # in case of groups
